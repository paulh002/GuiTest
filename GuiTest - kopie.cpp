...
public:
    ...
    bool initialize() {
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            std::cout << "🎯 Window " << i+1 << " assigned to: " << output_names[i] << "\n";
    ...
    void run() {
        ...
        while (running) {
            // Always flush pending events before polling
            wl_display_dispatch_pending(display);

            // Flush outgoing requests
            wl_display_flush(display);

            // Poll for events with 3-second timeout
            ...
            if (ret == 0) {
                // TIMEOUT: 3 seconds passed → update colors
                ...
                // Dispatch any pending events after commit
                wl_display_dispatch_pending(display);
            } else {
                // Events ready → dispatch them safely
